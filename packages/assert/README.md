...
The pillar-agnostic **assertion operator core** shared across Strummer — so there
is one assertion vocabulary for both the API engine (`@strummer/api`) and the
browser engine (`@strummer/browser`).
...
```ts
import { applyOp, type AssertionOp } from '@strummer/assert'
