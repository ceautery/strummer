...
is one assertion vocabulary for both the API engine (`@sackville/api`) and the
browser engine (`@sackville/browser`).
...
```ts
import { applyOp, type AssertionOp } from '@sackville/assert'
