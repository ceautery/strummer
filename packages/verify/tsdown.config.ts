...
// `@strummer/verify` is the run-driving orchestration layer (ADR 0013 Addendum,
// milestone 5c). It TYPE-imports the pillar result interfaces and RUNTIME-imports
// only the pure `@strummer/verdict` — never an engine runtime. Keep all
// `@strummer/*` external: the emitted `.mjs` then references only `@strummer/verdict`
