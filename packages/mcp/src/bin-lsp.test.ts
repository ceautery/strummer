...
describe('buildLspServerFromEnv', () => {
  ...
  it('enables the navigation tools with the full gate (allowRun + roots + registry)', async () => {
    const built = buildLspServerFromEnv({
      ...
      SACKVILLE_LSP_SERVERS: SERVERS,
      SACKVILLE_LSP_TIMEOUT_MS: '15000',
      SACKVILLE_LSP_MAX_SERVERS: '4',
      SACKVILLE_LSP_IDLE_TTL_MS: '600000',
    })
  ...
  it('parses SACKVILLE_LSP_ALLOW_WRITE (still exposes lsp_rename — apply is the engine decision)', async () => {
    const built = buildLspServerFromEnv({
      SACKVILLE_LSP_ALLOW_RUN: '1',
      SACKVILLE_LSP_ALLOW_WRITE: '1',
      SACKVILLE_LSP_PROJECT_ROOTS: '/abs/project',
      SACKVILLE_LSP_SERVERS: SERVERS,
    })
  ...
  it('parses SACKVILLE_LSP_ALLOW_PARTIAL_RENAME (default off — suspect renames refused for write)', () => {
    ...
    const on = buildLspServerFromEnv({
      SACKVILLE_LSP_ALLOW_RUN: '1',
      SACKVILLE_LSP_ALLOW_WRITE: '1',
      SACKVILLE_LSP_ALLOW_PARTIAL_RENAME: '1',
      SACKVILLE_LSP_PROJECT_ROOTS: '/abs/project',
      SACKVILLE_LSP_SERVERS: SERVERS,
    })
  ...
  it('parses SACKVILLE_LSP_ALLOW_DESTRUCTIVE_RESOURCE_OPS (default off)', () => {
    const off = buildLspServerFromEnv({
      SACKVILLE_LSP_ALLOW_RUN: '1',
      SACKVILLE_LSP_PROJECT_ROOTS: '/abs/project',
      SACKVILLE_LSP_SERVERS: SERVERS,
    ...
    const on = buildLspServerFromEnv({
      SACKVILLE_LSP_ALLOW_RUN: '1',
      SACKVILLE_LSP_ALLOW_WRITE: '1',
      SACKVILLE_LSP_ALLOW_DESTRUCTIVE_RESOURCE_OPS: '1',
      SACKVILLE_LSP_PROJECT_ROOTS: '/abs/project',
      SACKVILLE_LSP_SERVERS: SERVERS,
    })
  ...
  it('HARD-ERRORS when allowDestructiveResourceOps is set without allowWrite', () => {
    ...
    ).toThrow(/SACKVILLE_LSP_ALLOW_DESTRUCTIVE_RESOURCE_OPS requires SACKVILLE_LSP_ALLOW_WRITE/)
  })
  ...
  it('HARD-ERRORS when allowWrite is set without allowRun (cannot write without a live server)', () => {
    ...
    ).toThrow(/SACKVILLE_LSP_ALLOW_WRITE requires SACKVILLE_LSP_ALLOW_RUN/)
  })

  it('a registry without allowRun keeps navigation OFF (gate is paired)', async () => {
    const built = buildLspServerFromEnv({ SACKVILLE_LSP_SERVERS: SERVERS })
  ...
  it('fails loud on a malformed registry (operator misconfiguration is not silent)', () => {
    expect(() => buildLspServerFromEnv({ SACKVILLE_LSP_SERVERS: '{not json' })).toThrow()
  })
})
