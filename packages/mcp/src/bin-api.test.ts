...
describe('sackville-api-mcp bin config (operator env)', () => {
  ...
  it('SACKVILLE_BLOCK_PRIVATE hardens the SSRF posture', () => {
    expect(buildApiServerFromEnv({ SACKVILLE_BLOCK_PRIVATE: '1' }).config.allowPrivate).toBe(false)
  })
  ...
  it('enables the keyring secret chain only via SACKVILLE_KEYRING', () => {
    expect(buildApiServerFromEnv({ SACKVILLE_KEYRING: 'true' }).config.keyring).toBe(true)
