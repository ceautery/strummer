...
**`0.0.1-alpha.5` PUBLISHED — DONE (2026-06-06; via the OIDC pipeline). All 19 packages live on npm at alpha.5 with SLSA provenance.** Cut the alpha after the unreleased work piled up since alpha.4 (no changeset had been added for it): mutate Stryker partial-scope, flake diff-scoping, `verify_change` diffPath, the verify-CLI `--flow` unsafe/secret fix, and the shared browser capture-runtime builder. Process (RELEASING.md, all verified): already in pre-mode `alpha` → added changeset `alpha-5-diff-scoping-and-capture-runtime` (fixed group ⇒ all 19 bump) → pushed (`d7738d0`) → changesets action opened **Version PR #4** (all 19 → alpha.5, CHANGELOGs, pre.json) → squash-merged (`b2011d3`) → `release.yml` published over **OIDC, token-free** (provenance confirmed on `@sackville-mcp/verdict@0.0.1-alpha.5`); `changeset tag` pushed all 19 git tags. **`dist-tags` = `{ alpha: 0.0.1-alpha.5, latest: 0.0.1-alpha.4 }`** — pre-mode never moves `latest`, so a bare `npm i sackville-mcp` still resolves to alpha.4 until `latest` is repointed (manual `npm dist-tag`, needs a token / the npm UI — the unscoped `sackville-mcp` is user-owned; NOT done here, no token in this env). NEXT: repoint `latest` alpha.4→alpha.5 (operator, optional); branch protection (operator, PAT lacks Administration); or a deferred tail.

**BROWSER CAPTURE-RUNTIME — ONE SHARED BUILDER (2026-06-06; gate green 1703 TS + 48 Py; commit pending). Follow-up to the level-3 tutorial's dogfooding fix.** The drift that let `verify run --flow` ship without unsafe/secret wiring was structural: three surfaces independently constructed the browser capture core (gate + mandatory SSRF proxy + manager + `SACKVILLE_BROWSER_SECRET_*` redactor/resolver). Extracted ONE source of truth into `@sackville-mcp/browser` `runtime.ts` — pure `browserSecretsFromEnv(env)` (redactor+resolveSecret+secretNames) + `buildCaptureRuntime(opts)` (gate+proxy+manager+harDir+shutdown, surface-agnostic options). Refactored all three onto it: verify CLI `captureRuntimeFromFlags` (drops the stopgap `browserCaptureSecretsFromEnv`; keeps the pure `captureGateOptionsFromFlags` flag→opts mapper), browser CLI `withSession` (single-shot) + `cmdRun` (flow) — removed the dead `launchFor`, and `bin-browser`'s `buildBrowserRedactorFromEnv` now shares `browserSecretsFromEnv` for the secret loop (keeps its HTTP-credentials layer; its full server runtime stays a superset, NOT forced through the shared builder). 4 new browser tests (`runtime.test.ts`: pure secret parser + build/shutdown with no real browser — `createSsrfProxy` only binds a local port, as `proxy.test.ts` does). **All three surfaces re-verified LIVE against the storefront**: `browser snapshot` (ARIA), `browser run --unsafe` (flow PASS, secret resolved), `verify run --flow` (FAIL `contract: fail [high]`) — behavior preserved. NEXT: branch protection (operator, PAT lacks Administration); or a deferred tail (cosmic-ray cr-filter-git, mutmut cache reuse, src-layout reconcileMutmutScope precision, ecosystem changelog regex, LSP recursive/dir delete, toolchain matrix, pytest related-scoping for flake).

**LEVEL-3 ONBOARDING TUTORIAL (api + browser + verify) — DONE (2026-06-06; gate green 1700 TS + 48 Py; commit pending). Carries ONE real CLI bug fix surfaced by dogfooding.** A third runnable tutorial at `examples/tutorial/storefront/` exercising the pillars no tutorial covered: the **api** contract validator, the **browser** flow, and the cross-pillar **capture→contract** bridge (ADR 0013 §5e/5f). Design = ADR 0020 **addendum 2**. THE BUG: a **zero-dependency** Node `server.js` (`GET /account` + `/login`→`/dashboard` HTML; only prereq is Node, no `npm install`) returns `balance` as the STRING `'10000'` where `openapi.json` declares an **integer** (cents). The dashboard coerces it (`'10000'/100`), so the UI renders `$100.00` AND a login flow passes — yet the API violates its own contract. The deliberate contrast with tutorials 1–2: the bug is not a failing test / uncovered branch / unpinned boundary — it lives in the gap between what the API *sends* and what it *promised*, visible only to a contract-aware tool. Tool arc (all VERIFIED live via the built CLI against the running server): `search_docs` (balance=integer) → `api run --openapi` (every assertion PASS, `response contract: INVALID — response-schema: /balance must be integer`, exit 1) → `api validate-capture storefront.har.zip --openapi` (`NOT CLEAN`, 1× response-schema, exit 1; the committed HAR is a real capture of a *passing* flow) → `verify run --request … --openapi` (FAIL, `contract: fail [high]`, exit 1) → fix `balance:'10000'`→`10000` → `verify run` INCONCLUSIVE + `contract: pass` (exit 2, the absence-never-a-pass lesson, single pillar) → bonus live `verify run --flow login --allow-unsafe` (drives real chromium, FAIL). The committed `storefront.har.zip` (generated from the live server, Playwright-shaped inline-text bodies via fflate) makes steps 1–6 run with NO browser; only the bonus needs `npx playwright install chromium`. Two honesty guards: TS `test/tutorial-storefront.test.ts` (pins the bug via `validateOpenApiResponse` on the buggy body AND `validateCapturedTraffic` on the committed HAR → `response-schema`; corrected number body validates clean; docset/README sync) + pytest `test_tutorial_storefront.py` (ingests `storefront-core` docset offline, FTS `balance` finds the page). Sample is outside the pnpm workspace + root Vitest scope (never reddens the gate) but Biome-clean. **DOGFOODING FOUND + FIXED A REAL `@sackville-mcp/cli` BUG (TDD):** the verify **CLI**'s browser-capture runtime (`captureRuntimeFromFlags`) was built INCOMPLETE vs `sackville browser run` + the browser MCP server — it threaded NO `--allow-unsafe` (so a flow's `fill`/`click` DRY-RAN → any real flow failed the completeness guard → contract `no-signal (errored)`) AND wired NO secret resolver/redactor (so `{{secret:NAME}}` failed closed). Fixed via two pure exported helpers `captureGateOptionsFromFlags` + `browserCaptureSecretsFromEnv` (+3 unit tests); the MCP `verify_change` flow path was already correct (builds the full runtime from env). Wired references: root README (3rd tutorial), ADR 0020 addendum 2. NEXT: branch protection (operator, PAT lacks Administration); or a deferred tail (cosmic-ray cr-filter-git, mutmut cache reuse, src-layout reconcileMutmutScope precision, ecosystem changelog regex, LSP recursive/dir delete, toolchain matrix, pytest related-scoping for flake).

**FLAKE DIFF-SCOPING (vitest `related`) — DONE (2026-06-06; gate green 1668 TS + 47 Py; `19f0a5c` on `main`, pushed, CI green). Completes diff-scoping across ALL run-driving pillars (coverage ✓, mutate ✓ stryker+python, flake ✓ now).** flake's `runAndRecord` re-ran the WHOLE suite — no "given a change, repeat only the related tests" mode like coverage's `runScoped`. Worse, `verify_change`/`verify run` drove flake with `{files: changedFiles}` — changed SOURCE files fed as positional `vitest run` filters, which match no test file ⇒ flake **silently recorded NOTHING** (a latent no-signal in the composite's flake dimension). Added `related?: boolean` to `RunAndRecordInput`: vitest builds `vitest related <files> --run --reporter=json` (the EXACT invocation coverage's `runScoped` uses) so changed source files scope to their dependent tests; `related`+empty files = pre-spawn noop (`ran:false`); pytest REFUSES `related` (its mirrored-test scope stays staged — `supportsRelated:false`, throws). Surfaced on MCP `flake_run.related` + CLI `flake run --related`. **Fixed verify (bin-verify + cli verify) to drive flake with `related: changedFiles.length>0`** so the composite's flake dimension actually exercises the change's tests (no change set ⇒ whole suite, unchanged). 8 net new tests across engine/MCP/CLI. **Dogfooded via the native `mcp__sackville__run_scoped --diffPath` over all 5 changed source files** (all new executable lines covered, `uncovered: []`). Design = ADR 0010 addendum 2 (flake diff-scoping un-staged; pytest `related` still staged). NEXT: branch protection (operator, PAT lacks Administration); staged web/API/browser tutorial; or another tail (cosmic-ray `cr-filter-git`, mutmut `mutants/` cache reuse, src-layout `reconcileMutmutScope` precision, ecosystem changelog regex, LSP recursive/dir delete, toolchain matrix, pytest related-scoping).

**`verify_change` `diffPath` INPUT — DONE (2026-06-06; gate green 1663 TS + 47 Py; `329a5b7`, pushed, CI green). A dogfooding-surfaced API-consistency fix, built USING the native MCP tools.** The agent-surface `verify_change` accepted only an inline `diff`, while `run_scoped` accepts `diff` OR `diffPath` — so an agent that scoped `run_scoped` by `diffPath` and reused it for `verify_change` **silently got a no-signal coverage verdict** (the diff feeds `changedFiles` + the deps ecosystem scope) instead of a real one. Now `verify_change` resolves `args.diff ?? readFileSync(args.diffPath)` (inline wins), mirroring `run_scoped` — one new optional input, otherwise no behavior change. The CLI `sackville verify run --diff <path>` already took a PATH, so this was a ONE-surface (MCP) gap; no CLI change. 2 new tests (diffPath derives changedFiles; inline wins over diffPath). **Dogfooded via the now-NATIVE `mcp__sackville__run_scoped` with `--diffPath` on this very change** (the executable new line covered, `uncovered: []`). Closes dogfooding finding #5 (see [[sackville-dogfooding-mcp]]). NEXT: branch protection (operator, PAT lacks Administration); staged web/API/browser tutorial; or another tail (flake diff-scoping, ecosystem changelog regex, LSP recursive/dir delete, toolchain matrix).

**STRYKER PARTIAL-SCOPE RECONCILIATION — DONE (2026-06-06; gate green 1661 TS + 47 Py; HEAD `fba11f3` on `main`, pushed). Built while DOGFOODING the Sackville MCP server against this repo.** Closes the last mutate diff-scoping asymmetry: `runMutation` (Stryker) passed `mutateFiles` straight to `--mutate` and returned `scopedFiles = requested`, with NO selection or post-spawn reconciliation — so a changed source file Stryker silently never mutated (excluded by its config / no glob match) read as a clean pass (absence-as-a-pass), the exact hole `reconcileScope` already closed for cosmic-ray/mutmut (STATUS noted "Stryker PARTIAL-scope reconciliation staged"). Now `runMutation` mirrors `runCosmicRay`. **SLICE-0 capture first** (the repo's no-doc-guesses discipline): real **Stryker 9.6.1 OMITS a zero-mutant `--mutate` file from its JSON report's `files` map** (verified via the scheduler-sample Stryker on a type-only probe file) — the **mutmut Fork-B2 shape, NOT cosmic-ray's seen-but-empty** — so `reconcileScope` is **REUSED as-is**: a selected file ABSENT from the report ⇒ `missing` ⇒ throw inconclusive (conservative, **ZERO false-passes**; the precision loss is inherent to Stryker's report format, not a code defect). `selectMutationScope` generalized with an optional `isMutableSource` predicate (default `.py`; Stryker passes JS/TS excluding `.d.ts`+test files — both reliably zero-mutant, would else false-flag) + an `ownedRoots` entry of `'.'` = whole-project (the Stryker default, since diff files are in-repo); both backward-compatible for the Python callers. Scoped path: select → noop-if-empty (`ran:false`/`scopeEmpty`, never spawns) → `--mutate` the selected set → reconcile → `scopedFiles = mutatedFiles` + `requestedFiles` + `unmatched`; whole-project path (`mutateFiles===undefined`) byte-identical. The `scopedFiles` semantic change (requested→actually-mutated) + the existence filter broke two CONSUMER tests (`mcp`/`cli` `mutate_run`) — **surfaced automatically by dogfooding coverage `run_scoped` (vitest related across packages), NOT by `lsp_find_references` on the engine symbol** (which sees only in-package refs) — fixed to use a real temp project. 8 net new tests. Design = ADR 0010 addendum 2 (Stryker half un-staged). **DOGFOODING the MCP server (the session's experiment) surfaced 5 rough edges (see [[sackville-dogfooding-mcp]]):** (1) no committed `.mcp.json` — the repo doesn't dogfood by default; (2) stale local CLI build crashes on the alpha.4 `@sackville-mcp/spawn` import until `pnpm -r build`; (3) mid-session `claude mcp add` doesn't surface tools until a Claude Code reconnect (toolsets bind at startup) — drove the server via raw MCP JSON-RPC instead; (4) `run_scoped` catches behavioral cross-package blast radius that `lsp_find_references` (symbol-only) misses — complementary, not redundant; (5) **`verify_change` accepts only inline `diff`, not `diffPath`** (which `run_scoped` DOES) → an agent reusing `diffPath` silently gets no-signal coverage (STAGED fix). The aggregate MCP server is verified working (24 tools across lsp/coverage/mutate/flake/verify/deps), wired at LOCAL scope pointing at `/workspace` (`.sackville/` gitignored). NEXT: branch protection (operator, still blocked here — the PAT lacks the *Administration* permission, so it must be done in the GitHub UI or with an Admin-scoped token); or the staged web/API/browser tutorial; or a deferred tail (incl. the `verify_change` `diffPath` fix, flake diff-scoping, ecosystem changelog regex, LSP recursive/dir delete, toolchain matrix).

**`0.0.1-alpha.4` PUBLISHED — DONE (2026-06-06; gate green 1653 TS + 47 Py; pushed). Fixed a CI false-green first.** The alpha.4 release shipped the `mutate-flake-runner-path` changeset (the `mutate`/`flake` runner-PATH fix + the `@sackville-mcp/spawn` extraction). **Before publishing, found CI had been RED on `main` since the spawn-extraction commit `a4c1ddb` (12:37) while the local gate false-greened:** `@sackville-mcp/spawn` was added as a `workspace:*` dep of coverage/mutate/flake but **never registered in `vitest.config.ts`'s source-alias map**. The green gate runs `vitest` BEFORE any build, so on a clean tree the import fell through to `exports.default`→`./dist/index.mjs`; locally a stale `packages/spawn/dist/` (left by a prior `pnpm -r build`) resolved it, masking the failure across six commits. CI failed `Failed to resolve entry for package "@sackville-mcp/spawn"`. The Release workflow only BUILDS (no gate) so it stayed green and hid it. **Fix (commit `a17e9ce`):** added the missing source alias + a regression guard in `test/packaging.test.ts` ("vitest source-alias coverage" — asserts every internally-consumed `@sackville-mcp/*` pkg has an alias; verified red without it, green with it; +1 test → 1653) + documented it as **step 0 of RELEASING.md "Adding a new package"**. Verified against a genuinely clean tree (removed `spawn/dist/` to reproduce). CI on `main` GREEN again (first green since 12:37). **Release:** merged Version PR #3 (squash `1e31abd`) → `release.yml` OIDC publish of all 19 at alpha.4, including **`@sackville-mcp/spawn`'s FIRST OIDC release (SLSA provenance confirmed live)** — the token-bootstrap→trusted-publisher handoff works. **`latest` repointed alpha.3→alpha.4 on all 19** (incl. the user-owned unscoped `sackville-mcp`, as `ceautery`); both `latest` + `alpha` now = alpha.4. All 19 git tags pushed by `changeset tag`. Standing operator item (branch protection on `main`, required checks `gate` + `package-checks`) is now MORE pointed — it would have blocked the red-CI commits from landing. See [[sackville-alpha-publish]] + RELEASING.md. NEXT: branch protection (operator), or a new direction (staged web/API/browser tutorial variant; a deferred technical tail).
...
**VALIDATED END-TO-END + a production bug fix it surfaced (2026-06-06; gate green 1651 TS + 47 Py).** A local runthrough of the scheduler tutorial's `mutate run` step (npm-installed the sample — Stryker **9.6.1** + vitest-runner 9.6.1 + vitest **4.1.8** + `@vitest/coverage-v8`; ran via the LOCAL `@sackville-mcp/cli` build) surfaced a real PATH bug AND let me get the tutorial's mutation story right. (1) **PRODUCTION FIX (changeset added, mirrors the alpha.3 coverage fix):** `sackville-cli mutate run` failed opaquely ("did not produce a JSON report") because `@sackville-mcp/mutate`'s `spawnMutationRunner` (and `@sackville-mcp/flake`'s `spawnRunner`) `execFile`'d a bare `stryker`/`vitest`/`pytest` with NO `env` → not on PATH from a global CLI. Added a pure, unit-tested `runnerEnv` (prepends `<cwd>/node_modules/.bin`) to BOTH packages, mirroring coverage's existing one (coverage was the only one fixed; mutate+flake had the identical latent gap). After rebuild, `mutate run` works: exit 0, score **80.0%**, **3 survivors at `src/interval.ts:24`** (see below). **(EXTRACTED — DONE:** `runnerEnv` + the identical `spawnRunner` wrapper + `SpawnedRunner` type now live in a NEW shared zero-dep package **`@sackville-mcp/spawn`** (19th package), consumed by coverage/mutate/flake — one source of truth, mirroring the safety/assert/diff/severity extraction discipline; the pillars keep their public `TestRunner`/`MutationRunner` names as aliases, so internal-only/no API change. Gate green 1652 TS + 47 Py; all 19 tarballs pass attw+publint + lazy-boundary; changeset updated. **`@sackville-mcp/spawn` is now PUBLISHED (operator bootstrap, 0.0.1-alpha.3, alpha+latest tags) + its trusted-publisher configured**, so the next OIDC release publishes it like the rest — npm can't first-publish a new package over OIDC (npm/cli#8544), so a new `@sackville-mcp/*` package needs a one-time token bootstrap THEN OIDC config; verified locally that `changeset version` bumps it with the fixed group to alpha.4 + auto-adds it to `pre.json.initialVersions` (no manual edit). Documented as RELEASING.md "Adding a new package"; counts there 18→19.) (2) **VERSIONS + MUTATION STORY corrected (the human flagged the stale pin):** my first pass back-versioned Stryker to ^8.7.1 from cutoff-era memory and added an `excludedMutations:["ConditionalExpression"]` workaround on a MISDIAGNOSIS — I'd claimed a "phantom/false survivor." It is NOT: dumping the mutant spans showed the surviving `ConditionalExpression [b.end < a.start] -> "false"` drops the SECOND `||` clause (`if(a.end<b.start || false)`), which survives LEGITIMATELY because every sample test puts interval `a` before `b`, so the second clause is never the deciding factor (the whole-condition `if(false)` mutant IS killed). Excluding it HID a real weak-test finding — backwards for a mutation tutorial. Fixed: bumped to Stryker **9.6.1** (latest; the behavior is identical, confirmed) + vitest **4.1.7** (aligns with the repo/todo tutorial; vitest-runner 9.6.1 peers `vitest>=2`) + added `@vitest/coverage-v8` (the README's `--coverage` step needed it). `stryker.config.json` is now MINIMAL (no `coverageAnalysis`/`excludedMutations` overrides). Validated: pristine = **3 survivors** (2 boundary `<`→`<=` `EqualityOperator` + 1 `ConditionalExpression` "only one ordering tested"); fix + a BOTH-ORIENTATIONS touching test (`overlaps(a,b)` AND `overlaps(b,a)`) = **15 killed, 0 survived** → clean verify. README step 4 now describes all three honestly. package-lock.json gitignored (todo convention). **FULL CLI RUNTHROUGH (steps 0–6) done end-to-end against the local builds (`node packages/cli/dist/bin.mjs` + uv ingester + real `typescript-language-server` 5.3.0 + Stryker 9.6.1):** steps 0–5 all match the README (symptom → `search`/`get` → `lsp definition`/`references` cross-file blast radius → coverage-says-covered → `mutate run` 3 survivors → fix+both-orders test → 0 survivors). **Step 6 surfaced a DOCS BUG in BOTH tutorials:** `verify run <one-pillar>` returns **INCONCLUSIVE (exit 2)**, NOT the `pass` (exit 0) both READMEs claimed — because `composeVerdict` (compose.ts:41) ALWAYS folds all 5 pillars and treats unrequested ones as `missing`→inconclusive. This is INTENDED + load-bearing per ADR 0013 (§slice 7 empty-fold=inconclusive "corrected from the draft"; §5c not-requested folds to inconclusive "for free"; §164 the per-pillar breakdown is the primary output, the composite goes green only when every dimension is affirmatively checked). Fixed BOTH READMEs (scheduler step 6 + todo step 4) to show the real per-pillar breakdown + explain exit 2 is correct for a single-pillar run (the `mutate: pass`/`coverage: pass` line is the proof); illustrative output strings verified against the real run + the `fromDiffCoverage` headline. NOT a code change — verify works as designed. Also added `docs/mutation-testing.md` (history + the 3 engines; citations source-verified) linked from the README's `mutate` reference. See [[sackville-tutorial-runthrough-fixes]] + [[sackville-onboarding-docs]]. **NEXT (pending work on `main`, unreleased):** there is an unconsumed changeset (`mutate-flake-runner-path`, which now also carries the spawn extraction), so an **`alpha.4` release is due** — merge a "Version Packages (alpha)" PR (confirm `@sackville-mcp/spawn` lists at alpha.4 + the new package publishes over OIDC now that its trusted-publisher is set). Standing operator item: branch protection on `main` (required checks: `gate` + `package-checks`). Then pick a new direction (the staged web/API/browser tutorial variant; or a deferred technical tail).
