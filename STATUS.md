...
**`0.0.1-alpha.5` PUBLISHED — DONE (2026-06-06; via the OIDC pipeline). All 19 packages live on npm at alpha.5 with SLSA provenance.** Cut the alpha after the unreleased work piled up since alpha.4 (no changeset had been added for it): mutate Stryker partial-scope, flake diff-scoping, `verify_change` diffPath, the verify-CLI `--flow` unsafe/secret fix, and the shared browser capture-runtime builder. Process (RELEASING.md, all verified): already in pre-mode `alpha` → added changeset `alpha-5-diff-scoping-and-capture-runtime` (fixed group ⇒ all 19 bump) → pushed (`d7738d0`) → changesets action opened **Version PR #4** (all 19 → alpha.5, CHANGELOGs, pre.json) → squash-merged (`b2011d3`) → `release.yml` published over **OIDC, token-free** (provenance confirmed on `@sackville-mcp/verdict@0.0.1-alpha.5`); `changeset tag` pushed all 19 git tags. **`latest` repointed alpha.4→alpha.5 on ALL 19** (2026-06-06, manual `npm dist-tag add` as `ceautery` — the operator supplied a publish token; pre-mode never moves `latest` automatically). Confirmed `{ latest: 0.0.1-alpha.5, alpha: 0.0.1-alpha.5 }` across the unscoped `sackville-mcp` + all 18 scoped, so a bare `npm i` / `npx -y sackville-mcp` now gets alpha.5. NEXT: branch protection (operator, PAT lacks Administration); or a deferred tail.
