...
**VALIDATED END-TO-END + a production bug fix it surfaced (2026-06-06; gate green 1651 TS + 47 Py).** A local runthrough of the scheduler tutorial's `mutate run` step (npm-installed the sample — vitest 3.2.6, Stryker 8.7.1, vitest-runner 8.7.1; ran via the LOCAL `@sackville-mcp/cli` build) surfaced a real PATH bug AND let me tighten the tutorial. (1) **PRODUCTION FIX (changeset added, mirrors the alpha.3 coverage fix):** `sackville-cli mutate run` failed opaquely ("did not produce a JSON report") because `@sackville-mcp/mutate`'s `spawnMutationRunner` (and `@sackville-mcp/flake`'s `spawnRunner`) `execFile`'d a bare `stryker`/`vitest`/`pytest` with NO `env` → not on PATH from a global CLI. Added a pure, unit-tested `runnerEnv` (prepends `<cwd>/node_modules/.bin`) to BOTH packages, mirroring coverage's existing one (coverage was the only one fixed; mutate+flake had the identical latent gap). After rebuild, `mutate run` works: exit 0, score 81.8%, **2 EqualityOperator survivors at `src/interval.ts:24`** — exactly the boundary bug. (NOTE: 3 copies of the 5-line `runnerEnv` now exist — coverage/mutate/flake; an extraction to a shared zero-dep helper is a reasonable follow-up, deferred to avoid a new-package detour for 5 lines.) (2) **TUTORIAL CONFIG tightened:** the Stryker+vitest-runner mis-reports the `ConditionalExpression→true/false` mutant as *survived* even when a test catches it (a false survivor — confirmed always-true `overlaps` DOES fail the disjoint test) → `stryker.config.json` now sets `coverageAnalysis:off` + `mutator.excludedMutations:["ConditionalExpression"]` so the survivors are EXACTLY the two boundary `<`→`<=` `EqualityOperator` mutants. Validated: pristine = 2 boundary survivors; fix + a BOTH-ORIENTATIONS touching test (`overlaps(a,b)` AND `overlaps(b,a)` — the disjoint check has a clause per operand) = 0 survivors → clean verify. README updated to guide both orientations + "the boundary comparison survives" (not "one survives"). package-lock.json gitignored (todo convention). See [[sackville-tutorial-runthrough-fixes]]. NEXT: pick a new direction with the human (the staged web/API/browser tutorial variant; the operator branch-protection item; a deferred tail — incl. extracting the shared `runnerEnv`).
