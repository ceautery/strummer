# CLAUDE.md — how we build Sackville
...
5. **Commit directly to `main`. Push to GitHub at milestones** (not every
   commit). Remote: `https://github.com/ceautery/sackville`.
...
- `packages/` — TS workspace: `core` (docs domain + SQLite), `embed` (query
  ...
  the verify MCP bin AND the `sackville verify run --flow` CLI)),
  ...
  store — `sackville://<prefix>/<id>/<kind>` by-handle egress, parameterized prefix;
  ...
  the long-running server bins via `SACKVILLE_<PILLAR>_ARTIFACT_MAX_*`; no policy ⇒ no GC),
  ...
  verify selector via `SACKVILLE_MUTATE_TOOL`/`--mutate-tool` (Fork D)),
  ...
  slices A–G): dry-run by default + a SEPARATE `SACKVILLE_LSP_ALLOW_WRITE` gate enforced to require
  ...
  `sackville lsp` CLI DONE. Dynamic `didChangeWorkspaceFolders` DONE (grow-only warm-server reuse: a
  ...
  verify-DRIVEN live capture (browser-spawn):** `verify_change`'s `contract` input + `sackville verify run
...
- `py/sackville_ingest/` — Python ingester (uv).
...
- The Linux dev-container harness (`docker/`, `docker-compose.yml`) that hosts
  ...
  container, not part of Sackville.
